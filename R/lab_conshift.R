#' @title Risk estimation and inference under concept shift in label/outcome
#' @name lab_conshift
#' @export
#' @description Using data from both source population (feature/covariate may be missing) and target population (fully observed), estimate the risk of a list of learners in the target population under concept shift (in label/outcome) assumption (that is, label/outcome distributions are identical in both populations but the distributions of feature/covariate given label/outcome might differ). The estimator is semiparametrically efficient when the nuisance function is consistently estimated and is asymptotically normal even if the nuisance function is inconsistently estimated.
...
#' See \link{covshift}
lab_conshift<-function(learners,loss.fun,A,X,Y,nfolds=5,
    ...
    for(learner.index in 1:nlearners){
        ...
        cond.loss<-CV.target.est(A,Y,as.matrix(loss),folds,cond.loss.SL.control)
